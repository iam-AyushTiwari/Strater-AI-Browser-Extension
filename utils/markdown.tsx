...
export default function Markdown({ markdown, className }: MarkdownProps) {
  ...
  const convertToHtml = (markdownText: string) => {
    ...
    markdownText = markdownText.replace(/^### (.*$)/gim, '<h3 class="text-xl font-semibold">$1</h3>')
    markdownText = markdownText.replace(/^## (.*$)/gim, '<h2 class="text-2xl font-semibold">$1</h2>')
    markdownText = markdownText.replace(/^# (.*$)/gim, '<h1 class="text-3xl font-bold mb-4">$1</h1>')
    ...
    markdownText = markdownText.replace(/^(?:-|\+|\*)\s+(.+)$/gim, '<li class="list-disc ml-5 text-[18px]">$1</li>')
    markdownText = markdownText.replace(/((?:<li class="list-disc ml-5">.+<\/li>\s*)+)/gim, '<ul class="list-inside">$1</ul>')

    // Convert ordered lists
    markdownText = markdownText.replace(/^\d+\.\s+(.+)$/gim, '<li class="list-decimal ml-5 text-[18px]">$1</li>')
    ...
    markdownText = markdownText.replace(/^(?!<(?:ul|ol|li|h1|h2|h3|h4|h5|h6|blockquote|a|strong|i|em|div|p)\b)[^<]+/gim, '<p class="text-[18px]">$&</p>')
